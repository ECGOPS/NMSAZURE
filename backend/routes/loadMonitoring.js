...
router.get('/', requireRole(['system_admin', 'global_engineer', 'regional_engineer', 'project_engineer', 'district_engineer', 'regional_general_manager', 'district_manager', 'ict', 'technician']), async (req, res) => {
  try {
    console.log('[LoadMonitoring] GET request received with query:', req.query);
    console.log('[LoadMonitoring] Database:', databaseId);
    console.log('[LoadMonitoring] Container:', containerId);
    ...
    console.log('[LoadMonitoring] Executing query:', queryStr);
    const { resources } = await container.items.query(queryStr).fetchAll();
    console.log('[LoadMonitoring] Query result:', resources.length, 'records');
    res.json(resources);
  } catch (err) {
