...
if (process.env.NODE_ENV === 'development') {
  console.log('[MusicFiles] Endpoint:', endpoint);
}

// Helper: check if user exists in users collection
...
// GET all (read) - requires authentication and appropriate role
router.get('/', requireAuth(), requireRole(['system_admin', 'global_engineer', 'regional_engineer', 'project_engineer', 'district_engineer', 'regional_general_manager', 'district_manager', 'ict', 'technician']), async (req, res) => {
  ...
  // }
  try {
    console.log('[MusicFiles] GET request received');
    const { resources } = await container.items.query('SELECT * FROM c').fetchAll();
    res.json(resources);
  } catch (err) {
    console.error('[MusicFiles] Error:', err);
...
router.post('/', requireAuth(), requireRole(['system_admin']), async (req, res) => {
  ...
  // }
  try {
    console.log('[MusicFiles] POST request received:', req.body);
...
router.put('/:id', requireAuth(), requireRole(['system_admin']), async (req, res) => {
  ...
  try {
    const { id } = req.params;
    console.log('[MusicFiles] PUT request received for ID:', id, 'Data:', req.body);
...
router.delete('/:id', requireAuth(), requireRole(['system_admin']), async (req, res) => {
  ...
  try {
    const { id } = req.params;
    console.log('[MusicFiles] DELETE request received for ID:', id);
