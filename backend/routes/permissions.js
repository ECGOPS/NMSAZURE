...
if (process.env.NODE_ENV === 'development') {
  console.log('[Permissions] Endpoint:', endpoint);
}
...
router.get('/', requireAuth(), requireRole(['system_admin', 'global_engineer', 'regional_engineer', 'project_engineer', 'district_engineer', 'regional_general_manager', 'district_manager', 'ict', 'technician']), async (req, res) => {
  try {
    console.log('[Permissions] GET request received with query:', req.query);
    console.log('[Permissions] Database:', databaseId);
    console.log('[Permissions] Container:', containerId);
...
router.post('/', requireAuth(), requireRole(['system_admin']), async (req, res) => {
  try {
    console.log('[Permissions] POST request received:', req.body);
...
router.put('/:id', requireAuth(), requireRole(['system_admin']), async (req, res) => {
  try {
    const { id } = req.params;
    console.log('[Permissions] PUT request received for ID:', id, 'Data:', req.body);
...
router.delete('/:id', requireAuth(), requireRole(['system_admin']), async (req, res) => {
  try {
    const { id } = req.params;
    console.log('[Permissions] DELETE request received for ID:', id);
