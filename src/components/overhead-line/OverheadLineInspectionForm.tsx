  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.images && formData.images.length > 0) {
        console.log(`📸 Uploading ${formData.images.length} before photos...`);
      ...
      if (formData.afterImages && formData.afterImages.length > 0) {
        console.log(`📸 Uploading ${formData.afterImages.length} after photos...`);
  ...
  useEffect(() => {
    const fetchFeeders = async () => {
      ...
      try {
        ...
        // Use FeederService for offline support - get all feeders first, then filter
        ...
        if (filteredFeeders.length > 0 && !formData.feederName) {
          const firstFeeder = filteredFeeders[0];
          setFormData(prev => ({
            ...prev,
    ...
    fetchFeeders();
  }, [formData.region, regions, feederService, user]);
            ...
            >
              ...
              <SelectContent>
                {feeders.map((feeder, index) => (
                  <SelectItem key={`${feeder.id}-${index}`} value={feeder.name}>
