...
export function VITAssetForm({ asset, onSubmit, onCancel }: VITAssetFormProps) {
  ...
  useEffect(() => {
    ...
    // Set region for district engineers and other roles
  ...
  const filteredDistricts = (() => {
    ...
    // For regional engineers and regional general managers, filter by their region
    if (user?.role === "regional_engineer" || user?.role === "regional_general_manager") {
      if (user?.region) {
        const userRegion = regions.find(r => r.name === user.region);
        if (userRegion) {
          return filtered.filter(d => d.regionId === userRegion.id);
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (asset) {
        ...
        if (isOnline) {
          await apiRequest('/api/vitAssets', {
            method: 'POST',
            body: JSON.stringify(assetData),
          });
          ...
          const offlineAsset = await apiRequest('/api/vitAssets', {
            method: 'POST',
            body: JSON.stringify(assetData),
          });
          ...
          window.dispatchEvent(new CustomEvent('assetAdded', {
            detail: {
              ...
              asset: {
                ...assetData,
                id: offlineAsset.id, // Assuming the API returns the created asset with an ID
                syncStatus: 'created'
              },
              status: 'success'
            }
