...
export const AzureADAuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const logout = async (navigateToLogin?: () => void) => {
    try {
      ...
      // ✅ Step 1: Clear all session data first
      await clearAllSessionData();
      ...
      // ✅ Step 3: Show logout message
      toast.success("You have been logged out");
      
      // ✅ Step 4: Prepare logout redirect URL with force re-authentication
      const postLogoutRedirectUri = `${
        import.meta.env.VITE_AZURE_AD_POST_LOGOUT_REDIRECT_URI || window.location.origin
      }/login?logout=true&forceReLogin=true`;
      
      // ✅ Step 5: Perform MSAL logout with force re-authentication
      const account = getAccount();
      if (account) {
        ...
        // Fallback if no account is found
        await msalInstance.clearCache();
