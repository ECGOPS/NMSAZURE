...
export class OfflineStorageService {
    ...
    try {
      this.db = await openDB<DBSchemaWithOffline>(this.DB_NAME, this.DB_VERSION, {
        upgrade(db) {
          ...
          if (!db.objectStoreNames.contains('vitAssets')) {
            db.createObjectStore('vitAssets', { keyPath: 'id' });
          }
          if (!db.objectStoreNames.contains('vitInspections')) {
            db.createObjectStore('vitInspections', { keyPath: 'id' });
          }
  ...
  public async saveVITAsset(asset: any): Promise<void> {
    if (!this.db) {
      await this.initialize();
    }

    await this.db!.put('vitAssets', asset);
  }

  public async getVITAssets(): Promise<any[]> {
    if (!this.db) {
      await this.initialize();
    }

    return await this.db!.getAll('vitAssets');
  }

  public async saveVITInspection(inspection: any): Promise<void> {
    if (!this.db) {
      await this.initialize();
    }

    await this.db!.put('vitInspections', inspection);
  }

  public async getVITInspections(): Promise<any[]> {
    if (!this.db) {
      await this.initialize();
    }

    return await this.db!.getAll('vitInspections');
  }
