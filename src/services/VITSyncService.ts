...
export class VITSyncService {
  ...
  public async getPendingVITAssets(): Promise<VITAsset[]> {
    try {
      ...
      const offlineAssets = await this.offlineStorage.getVITAssets();
      console.log('[VITSync] Found offline assets:', offlineAssets.length);
      return offlineAssets.filter(asset => 
        asset.syncStatus === 'created' || 
        asset.syncStatus === 'updated' || 
